...
def labels(canvas):
    # month/year label
    canvas.create_text(200, 620, anchor='w', font='Courier 52 bold', text='June 2020')
    # days of the week
    for i in range(7):
        canvas.create_text(15 + i * SIZE_COL, 15, anchor='w', font='Courier 10 bold', text=days[i])
    ...
    for i in range(1, 8):
        ...
        for key in june_birthdays.keys():
            if june_birthdays[key].day == i:
                canvas.create_text(10 + i * SIZE_COL, 95, anchor='w', font='Courier 7', text=key)
    # middle 3 weeks
        ...
        for col in range(N_COLS):
            ...
            for key in june_birthdays.keys():
                if june_birthdays[key].day == col+7*(row-1):
                    canvas.create_text(10 + col * SIZE_COL, row * 101, anchor='w', font='Courier 7', text=key)
    # last week
    for i in range(3):
        canvas.create_text(67 + i * SIZE_COL, 560, anchor='w', font='Courier 9', text=str(i + 28))
        for key in june_birthdays.keys():
            if june_birthdays[key].day == i + 28:
                canvas.create_text(10 + i * SIZE_COL, 540, anchor='w', font='Courier 7', text=key)
    return canvas
